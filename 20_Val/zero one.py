...
float_data[:,1:-1]=dp.zero_one_normalization(float_data[:,1:-1])
data=float_data[:,1:-1]

data[150:]=K.get_value(K.dropout(data[150:],0.0001,None,None))
train_data=data[150:]
...
plt.plot(epochs, acc_20_MCAR, 'bo', label='Training acc Active Drop 30')
plt.plot(epochs, val_acc_20_MCAR, 'b', label='Validation acc Active Drop 30')
plt.plot(epochs, acc_20_NAD, 'go', label='Training acc Mean Imp')
plt.plot(epochs, val_acc_20_NAD, 'g', label='Validation  acc Mean Imp')
plt.xlabel('Epochs')
plt.ylabel('Accuracy')
plt.title('Training and Validation accuracy 20% Missing Data')
plt.legend()
plt.show()
'''
