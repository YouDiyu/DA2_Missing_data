...
class data_preprocess():
    ...
    def elim(self,data):
        elim=data.dropna()
        return elim
    def Merge(self,train_data,label_data):
        data=np.zeros((len(train_data),len(train_data[0])+1))
        data[:,:len(train_data[0])]=train_data
        data[:,len(train_data[0])]=label_data
        return data
    def zerotonan(self,train_data):
        train_data=(train_data/train_data)*train_data
        return train_data
    def verarbeit_elim(self,train_data,label_data):
        train_data=self.zerotonan(train_data)
        data=self.Merge(train_data,label_data)
        dropd=self.elim(self.to_pandas(data))
        train_data=np.array(dropd)[:,:-1]
        label_data=np.array(dropd)[:,-1]
        return train_data,label_data
    
    def shuffel(self,data):
        data=pd.DataFrame(data)
        data.sample(frac=1.0)
        data=np.array(data)
        return data
    def verarbeit_meanimp(self,train_data,label_data):
        train_data=self.zerotonan(train_data)
        data=self.Merge(train_data,label_data)
        ...
        label_data=data[:,-1]
        return train_data,label_data
    def verarbeit_mulimp(self,train_data,label_data,m=5):
        train_data=self.zerotonan(train_data)
        data=self.Merge(train_data,label_data)
        data_mi=self.multi_imp(data,m)
        data_mi=self.shuffel(data_mi)
    ...
    def zero_mean_normalization(self,train_data,weighting=False):
        ...
        if weighting:
            ...
            return processed_data
    def zero_one_normalization(self,train_data,weighting=False):
        #zero-mean-normalization
        mask_binary=self.generate_mask(train_data)
        #nan replace with zero
        train_data_zero=train_data.copy()
