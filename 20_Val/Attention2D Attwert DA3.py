...
history = model.fit(train_data_40_MCAR_elim, labe_data_40_MCAR_elim,validation_data=(data[:150,],float_data[:150,1]),epochs=150,batch_size=64)
...
plt.plot(epochs, val_acc1, 'r', label='Validation acc 20 MCAR')
plt.xlabel('Epochs')
plt.ylabel('Accuracy')
plt.ylim((0.5, 1))
...
def histshow(arr):
    ...
    plt.hist(arr, # 绘图数据
        ...
        label = 'Attention Werte' )# 为直方图呈现标签
    plt.xlabel('Feature')
    plt.ylabel('Value')
...
def barshow(arr):
    plt.style.use('bmh')
    plt.bar(range(len(arr)),arr, # 指定直方图的条形数为20个
        color = 'steelblue', # 指定填充色
        edgecolor = 'k', # 指定直方图的边界色
        label = 'Attention Werte' )# 为直方图呈现标签
    plt.xlabel('Feature')
...
# 显示图形
    plt.show()
attw=K.function([model.input],[model.layers[0].att])
atte=K.function([model.input],[model.layers[0].z])
outputs=atte=K.function([model.input],[model.layers[0].output])