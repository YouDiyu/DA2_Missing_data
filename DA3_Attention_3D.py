# -*- coding: utf-8 -*-
"""
Created on Tue Feb  5 21:35:19 2019
...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb  5 18:44:58 2019
...
#MAR
sort_float_data=sorted(float_data[150:], key=lambda x: x[20])
sort_float_data=np.array(sort_float_data)
sort_train_data=sort_float_data[:,2:]
label_data_MAR=sort_float_data[:,1]

data_20_MAR=train_data
a=K.dropout(sort_train_data[:200,:],0.15)
data_20_MAR[:200,:]=K.get_value(a)
b=a=K.dropout(sort_train_data[200:,:],0.25)
data_20_MAR[200:,:]=K.get_value(b)
...
a=K.dropout(sort_train_data[:200,:],0.3)
data_40_MAR[:200,:]=K.get_value(a)
b=a=K.dropout(sort_train_data[200:,:],0.5)
data_40_MAR[200:,:]=K.get_value(b)
...
train_data_voll_elim,label_data_voll_elim=np.expand_dims(train_data_voll,axis=-1),label_data
train_data_20_MCAR_elim,label_data_20_MCAR_elim=np.expand_dims(train_data_20_MCAR,axis=-1),label_data
train_data_40_MCAR_elim,labe_data_40_MCAR_elim=np.expand_dims(train_data_40_MCAR,axis=-1),label_data

train_data_20_MAR_elim,label_data_20_MAR_elim=np.expand_dims(train_data_20_MAR,axis=-1),label_data_20_MAR
train_data_40_MAR_elim,label_data_40_MAR_elim=np.expand_dims(train_data_40_MAR,axis=-1),label_data_40_MAR


def NNmodel(train_data,label_data,val_data1,val_data2):
    model = Sequential()
    model.add(You.GRU_M(8,return_sequences=True))
...
acc_voll, val_acc_voll=NNmodel(train_data_voll_elim,label_data_voll_elim,np.expand_dims(data[:150,],axis=-1),float_data[:150,1])

acc_20_MCAR, val_acc_20_MCAR=NNmodel(train_data_20_MCAR_elim,label_data_20_MCAR_elim,np.expand_dims(data[:150,],axis=-1),float_data[:150,1])
acc_40_MCAR, val_acc_40_MCAR=NNmodel(train_data_40_MCAR_elim,labe_data_40_MCAR_elim,np.expand_dims(data[:150,],axis=-1),float_data[:150,1]) 

acc_20_MAR, val_acc_20_MAR=NNmodel(train_data_20_MAR_elim,label_data_20_MAR_elim,np.expand_dims(data[:150,],axis=-1),float_data[:150,1])
acc_40_MAR, val_acc_40_MAR=NNmodel(train_data_40_MAR_elim,label_data_40_MAR_elim,np.expand_dims(data[:150,],axis=-1),float_data[:150,1]) 
